...
public class MonitoringInterceptorUtils {
    ...
    private static void addMonitoringPrefixedConfigs(final Properties config) {
        if (config.containsKey("bootstrap.servers")) {
            config.put(
                MonitoringInterceptorConfig.MONITORING_INTERCEPTOR_PREFIX + "bootstrap.servers",
                config.getProperty("bootstrap.servers"));
        }
        if (config.containsKey("security.protocol")) {
            config.put(
                MonitoringInterceptorConfig.MONITORING_INTERCEPTOR_PREFIX + "security.protocol",
                config.getProperty("security.protocol"));
        }
        if (config.containsKey(SaslConfigs.SASL_MECHANISM)) {
            config.put(
                MonitoringInterceptorConfig.MONITORING_INTERCEPTOR_PREFIX + SaslConfigs.SASL_MECHANISM,
                config.getProperty(SaslConfigs.SASL_MECHANISM));
        }
        if (config.containsKey(SaslConfigs.SASL_JAAS_CONFIG)) {
            config.put(
                MonitoringInterceptorConfig.MONITORING_INTERCEPTOR_PREFIX + SaslConfigs.SASL_JAAS_CONFIG,
        ...
        if (config.containsKey(SslConfigs.SSL_ENDPOINT_IDENTIFICATION_ALGORITHM_CONFIG)) {
            config.put(
                MonitoringInterceptorConfig.MONITORING_INTERCEPTOR_PREFIX + SslConfigs.SSL_ENDPOINT_IDENTIFICATION_ALGORITHM_CONFIG,
