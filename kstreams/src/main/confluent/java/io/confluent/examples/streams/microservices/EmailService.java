...
public class EmailService implements Service {
  ...
  private KafkaStreams processStreams(final String bootstrapServers,
    ...
    final KStream<String, Order> orders = builder.stream(Topics.ORDERS.name(),
        Consumed.with(Topics.ORDERS.keySerde(), Topics.ORDERS.valueSerde()));
    final KStream<String, Payment> payments = builder.stream(Topics.PAYMENTS.name(),
        Consumed.with(Topics.PAYMENTS.keySerde(), Topics.PAYMENTS.valueSerde()))
        //Rekey payments to be by OrderId for the windowed join
        .selectKey((s, payment) -> payment.getOrderId());
    final GlobalKTable<Long, Customer> customers = builder.globalTable(Topics.CUSTOMERS.name(),
        Consumed.with(Topics.CUSTOMERS.keySerde(), Topics.CUSTOMERS.valueSerde()));

    final StreamJoined<String, Order, Payment> serdes = StreamJoined
        .with(Topics.ORDERS.keySerde(), Topics.ORDERS.valueSerde(), Topics.PAYMENTS.valueSerde());
    ...
    orders.join(customers, (orderId, order) -> order.getCustomerId(), (order, customer) -> new OrderEnriched (order.getId(), order.getCustomerId(), customer.getLevel()))
        ...
        .to((orderId, orderEnriched, record) -> orderEnriched.getCustomerLevel(), Produced.with(Topics.ORDERS_ENRICHED.keySerde(), Topics.ORDERS_ENRICHED.valueSerde()));
