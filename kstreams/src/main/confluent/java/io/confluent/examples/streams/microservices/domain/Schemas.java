...
public class Schemas {
  ...
  public static Map<String, ?> buildSchemaRegistryConfigMap(final Properties config) {
    ...
    if (config.containsKey(AbstractKafkaSchemaSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG))
      map.put(AbstractKafkaSchemaSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, config.getProperty(AbstractKafkaSchemaSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG));
    if (config.containsKey(SchemaRegistryClientConfig.BASIC_AUTH_CREDENTIALS_SOURCE))
      map.put(SchemaRegistryClientConfig.BASIC_AUTH_CREDENTIALS_SOURCE, config.getProperty(SchemaRegistryClientConfig.BASIC_AUTH_CREDENTIALS_SOURCE));
    if (config.containsKey(SchemaRegistryClientConfig.USER_INFO_CONFIG))
      map.put(SchemaRegistryClientConfig.USER_INFO_CONFIG, config.getProperty(SchemaRegistryClientConfig.USER_INFO_CONFIG));
    return map;
  }
