...
public class FraudService implements Service {
  ...
  private KafkaStreams processStreams(final String bootstrapServers,
    ...
    final KTable<Windowed<Long>, OrderValue> aggregate = orders
        .groupBy((id, order) -> order.getCustomerId(), Grouped.with(Serdes.Long(), Topics.ORDERS.valueSerde()))
    ...
    forks.get("limit-above").mapValues(
        ...
        .to(Topics.ORDER_VALIDATIONS.name(), Produced
    ...
    final Properties props = MicroserviceUtils.baseStreamsConfig(bootstrapServers, stateDir, SERVICE_APP_ID, defaultConfig);
