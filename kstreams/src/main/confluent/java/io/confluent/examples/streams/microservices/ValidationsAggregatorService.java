...
public class ValidationsAggregatorService implements Service {
  ...
  private final Consumed<String, OrderValidation> serdes1 = Consumed
      .with(Topics.ORDER_VALIDATIONS.keySerde(), Topics.ORDER_VALIDATIONS.valueSerde());
  private final Consumed<String, Order> serdes2 = Consumed.with(Topics.ORDERS.keySerde(),
  ...
  private final StreamJoined<String, Long, Order> serdes4 = StreamJoined
      .with(Topics.ORDERS.keySerde(), Serdes.Long(), Topics.ORDERS.valueSerde());
  private final Produced<String, Order> serdes5 = Produced
      .with(Topics.ORDERS.keySerde(), Topics.ORDERS.valueSerde());
  ...
  private final StreamJoined<String, OrderValidation, Order> serdes7 = StreamJoined
      .with(Topics.ORDERS.keySerde(), Topics.ORDER_VALIDATIONS.valueSerde(), Topics.ORDERS.valueSerde());
  ...
  private KafkaStreams aggregateOrderValidations(
    ...
    final KStream<String, OrderValidation> validations = builder
        .stream(Topics.ORDER_VALIDATIONS.name(), serdes1);
    final KStream<String, Order> orders = builder
        .stream(Topics.ORDERS.name(), serdes2)
    ...
    validations
        ...
        .aggregate(
            () -> 0L,
            (id, result, total) -> OrderValidationResult.PASS.equals(result.getValidationResult()) ? total + 1 : total,
        ...
        .to(Topics.ORDERS.name(), serdes5);

    //If any rule fails then fail the order
    validations.filter((id, rule) -> OrderValidationResult.FAIL.equals(rule.getValidationResult()))
        .join(orders, (id, order) ->
        ...
        //Push the validated order into the orders topic
        .toStream().to(Topics.ORDERS.name(), Produced.with(Topics.ORDERS.keySerde(), Topics.ORDERS.valueSerde()));
  ...
  public static void main(final String[] args) throws Exception {
    ...
    service.start(
            cl.getOptionValue("bootstrap-servers", MicroserviceUtils.DEFAULT_BOOTSTRAP_SERVERS),
