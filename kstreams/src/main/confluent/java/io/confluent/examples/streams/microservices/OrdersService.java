...
public class OrdersService implements Service {
  ...
  private StreamsBuilder createOrdersMaterializedView() {
    ...
    builder.table(Topics.ORDERS.name(), Consumed.with(Topics.ORDERS.keySerde(), Topics.ORDERS.valueSerde()), Materialized.as(ORDERS_STORE_NAME))
  ...
  private void maybeCompleteLongPollGet(final String id, final Order order) {
    ...
    if (callback != null && callback.predicate.test(id, order)) {
      callback.asyncResponse.resume(OrderBean.toBean(order));
    }
  }
  ...
  private boolean locationMetadataIsUnavailable(final HostStoreInfo hostWithKey) {
    return StreamsMetadata.NOT_AVAILABLE.host().equals(hostWithKey.getHost())
        && StreamsMetadata.NOT_AVAILABLE.port() == hostWithKey.getPort();
  }
  ...
  public void submitOrder(final OrderBean order,
    ...
    final Order bean = OrderBean.fromBean(order);
    producer.send(new ProducerRecord<>(Topics.ORDERS.name(), bean.getId(), bean),
  ...
  public void start(final String bootstrapServers,
    ...
    producer = MicroserviceUtils.startProducer(bootstrapServers, Topics.ORDERS, defaultConfig);
  ...
  public static void main(final String[] args) throws Exception {
    ...
    final String bootstrapServers = cl.getOptionValue("bootstrap-servers", MicroserviceUtils.DEFAULT_BOOTSTRAP_SERVERS);
