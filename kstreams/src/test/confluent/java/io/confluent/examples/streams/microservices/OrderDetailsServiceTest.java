...
public class OrderDetailsServiceTest extends MicroserviceTestUtils {
  ...
  public void startKafkaCluster() throws Exception {
    ...
    config.put(AbstractKafkaSchemaSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, CLUSTER.schemaRegistryUrl());
  ...
  public void shouldPassAndFailOrders() throws Exception {
    ...
    orders = Arrays.asList(
        new Order(id(0L), 0L, OrderState.CREATED, Product.UNDERPANTS, 3, 5.00d), //should pass
        new Order(id(1L), 0L, OrderState.CREATED, Product.JUMPERS, -1, 75.00d) //should fail
    );
    sendOrders(orders);
  ...
  public void shouldCorrectlyProgressOffset() throws InterruptedException {
    ...
    orderValService = new OrderDetailsService();
    orderValService.start(CLUSTER.bootstrapServers(), TestUtils.tempDirectory().getPath(), new Properties());
    sendOrders(Collections.singletonList(new Order(id(0L), 0L, OrderState.CREATED, Product.UNDERPANTS, 3, 5.00d)));
    MicroserviceTestUtils.read(Topics.ORDER_VALIDATIONS, 1, CLUSTER.bootstrapServers()); //block

    //When sending second order
    sendOrders(Collections.singletonList(new Order(id(1L), 0L, OrderState.CREATED, Product.UNDERPANTS, 3, 5.00d)));

    //Then two orders should be validated
    assertThat(MicroserviceTestUtils.read(Topics.ORDER_VALIDATIONS, 2, CLUSTER.bootstrapServers()))
        .isEqualTo(Arrays.asList(
            new OrderValidation(id(0L), OrderValidationType.ORDER_DETAILS_CHECK, OrderValidationResult.PASS),
            new OrderValidation(id(1L), OrderValidationType.ORDER_DETAILS_CHECK, OrderValidationResult.PASS)
        ));

    //When sending third order
    sendOrders(Collections.singletonList(new Order(id(2L), 0L, OrderState.CREATED, Product.UNDERPANTS, 3, 5.00d)));
    ...
    assertThat(MicroserviceTestUtils.read(Topics.ORDER_VALIDATIONS, 3, CLUSTER.bootstrapServers()))
        .isEqualTo(Arrays.asList(
            new OrderValidation(id(0L), OrderValidationType.ORDER_DETAILS_CHECK, OrderValidationResult.PASS),
            new OrderValidation(id(1L), OrderValidationType.ORDER_DETAILS_CHECK, OrderValidationResult.PASS),
            new OrderValidation(id(2L), OrderValidationType.ORDER_DETAILS_CHECK, OrderValidationResult.PASS)
        ));
  }
