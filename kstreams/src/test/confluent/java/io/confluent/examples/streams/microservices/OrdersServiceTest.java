...
public class OrdersServiceTest extends MicroserviceTestUtils {
  ...
  public void shouldPostOrderAndGetItBack() {
    ...
    final Response response = postWithRetries(
      client.target(paths.urlPost()).request(MediaType.APPLICATION_JSON_TYPE),
      Entity.json(bean),
      5);
  ...
  public void shouldGetValidatedOrderOnRequest() {
    ...
    //When we post an order
    postWithRetries(client.target(paths.urlPost()).request(MediaType.APPLICATION_JSON_TYPE), Entity.json(beanV1), 5);

    //Simulate the order being validated
    MicroserviceTestUtils.sendOrders(Collections.singletonList(
      Order.newBuilder(orderV1)
  ...
  public void shouldTimeoutGetIfNoResponseIsFound() {
    ...
    try {
      ...
      Assert.fail("Request should have failed as materialized view has not been updated");
    } catch (final ServerErrorException e) {
      AssertionsForClassTypes.assertThat(e.getMessage()).isEqualTo("HTTP 504 Gateway Timeout");
    }
  }
  ...
  public void shouldGetOrderByIdWhenOnDifferentHost() {
    ...
    //And one order
    postWithRetries(client.target(paths1.urlPost()).request(MediaType.APPLICATION_JSON_TYPE), Entity.json(order), 5);
