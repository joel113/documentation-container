...
public abstract class AbstractGlobalStoresAndTablesTest {
    ...
    public void shouldDemonstrateGlobalStoreJoins() throws Exception {
        ...
        for (final EnrichedOrder enrichedOrder : enrichedOrders) {
            MatcherAssert.assertThat(customers, IsCollectionContaining.hasItem(enrichedOrder.getCustomer()));
            MatcherAssert.assertThat(products, IsCollectionContaining.hasItem(enrichedOrder.getProduct()));
            MatcherAssert.assertThat(orders, IsCollectionContaining.hasItem(enrichedOrder.getOrder()));
        }

        // demonstrate that global store data is available on all instances
        verifyAllCustomersInStore(customers, streamInstanceOne.store(StoreQueryParameters.fromNameAndType(CUSTOMER_STORE, QueryableStoreTypes.keyValueStore())));
        verifyAllCustomersInStore(customers, streamInstanceTwo.store(StoreQueryParameters.fromNameAndType(CUSTOMER_STORE, QueryableStoreTypes.keyValueStore())));
        verifyAllProductsInStore(products, streamInstanceOne.store(StoreQueryParameters.fromNameAndType(PRODUCT_STORE, QueryableStoreTypes.keyValueStore())));
        verifyAllProductsInStore(products, streamInstanceTwo.store(StoreQueryParameters.fromNameAndType(PRODUCT_STORE, QueryableStoreTypes.keyValueStore())));
    }
    ...
    private void verifyAllCustomersInStore(final List<Customer> customers,
        ...
        for (long id = 0; id < customers.size(); id++) {
            MatcherAssert.assertThat(store.get(id), CoreMatchers.equalTo(customers.get((int) id)));
        }
    }
    ...
    private void verifyAllProductsInStore(final List<Product> products,
        ...
        for (long id = 0; id < products.size(); id++) {
            MatcherAssert.assertThat(store.get(id), CoreMatchers.equalTo(products.get((int) id)));
        }
    }
